...
<Warning>
  **Video and audio materials are only supported by Seedance models** (`seedance-2.0`, `seedance-2.0`, `seedance-2.0-fast`). The `jimeng-video-*` models only support up to 2 images (first/last frame mode).
</Warning>
